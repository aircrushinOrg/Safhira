...
- Third‑party APIs: Geographic/topology data (TopoJSON/OpenStreetMap sources), optional analytics, and potential health/clinic datasets (future).
...
- For any LLM/AI features (present or future), mitigate prompt injection and jailbreaks; filter inputs/outputs and log usage for anomaly detection.
...
| PII Leakage in Chat Logs | Sensitive info inadvertently stored or exposed | 3 | 5 | 15 | PII redaction, data minimization, limited retention, encryption |
| Prompt Injection/LLM Abuse (if AI) | Model coerced to disclose or perform unsafe actions | 3 | 4 | 12 | Input/output filters, system prompt hardening, rate limits, monitoring |
...
| CORS Misconfiguration | Unauthorized origins can call APIs | 2 | 4 | 8 | Restrictive CORS, allowlist only, review in CI |
