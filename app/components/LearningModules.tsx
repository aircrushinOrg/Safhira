      ...
      <section className="py-16 px-4">
        <div className="container mx-auto max-w-4xl">
          <div className="mb-8">
            ...
            <p className="text-muted-foreground">Learning with Jun Kang - Accurate and reliable information</p>
          </div>

          <div className="space-y-8">
            {content.content.map((section, index) => (
              <Card key={index} className="p-8 bg-card/80 backdrop-blur-sm">
                <h3 className="text-xl font-semibold text-foreground mb-4">{section.section}</h3>
                <p className="text-muted-foreground mb-6">{section.text}</p>
          ...
          <div className="mt-12 text-center">
            <Card className="p-6 bg-gradient-to-r from-primary/10 to-secondary/10">
              <h4 className="font-semibold text-foreground mb-2">Finished reading?</h4>
              <p className="text-muted-foreground mb-4">Test your understanding with a quiz or ask AI if you have questions.</p>
              <div className="flex justify-center space-x-4">
                <Button onClick={() => onModuleClick('quiz')} className="bg-primary hover:bg-primary/90">
                  Take Quiz
                </Button>
      ...
      <div className="container mx-auto max-w-6xl">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-12">
          {modules.map((module, index) => (
            <Card key={module.id} className="p-6 bg-card/80 backdrop-blur-sm hover:shadow-lg transition-shadow cursor-pointer"
                  onClick={() => onModuleClick(module.id)}>
              <div className="flex items-start justify-between mb-4">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center bg-${module.color}/10`}>
              ...
              <p className="text-muted-foreground text-sm mb-4">{module.subtitle}</p>
              ...
              <div className="space-y-1 mb-4">
                {module.topics.slice(0, 2).map((topic, topicIndex) => (
                  <div key={topicIndex} className="text-xs text-muted-foreground flex items-center space-x-2">
